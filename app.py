...
def ensure_visit_count_once_per_session():
    ...
    if not st.session_state["visit_counted"]:
        _countapi_hit(VISIT_KEY)
        st.session_state["visit_counted"] = True

def load_usage_counts():
    return {
        "visits": _countapi_get(VISIT_KEY),
        "pdf_generations": _countapi_get(GENERATE_KEY),
    }
...
with right:
    ...
    st.markdown(
        ...
        4. Pasirink kainų režimą ir korekciją  
        5. Spausk **Generuoti PDF**  
        6. Po sugeneravimo pažymėk, ką nori parsisiųsti
        """
    )
...
with st.expander("Naudojimo statistika", expanded=False):
    ...
    gens = counts.get("pdf_generations")
    c1, c2 = st.columns(2)
    with c1:
        st.metric("Apsilankymai", visits if visits is not None else "—")
    with c2:
        st.metric("Sugeneruoti PDF", gens if gens is not None else "—")
    st.caption("Statistika kaupiama bendrai visiems naudojimams. Jei laikinas tinklo ryšys neveikia, skaičiai gali trumpam nerodyti reikšmių.")
    st.markdown('</div>', unsafe_allow_html=True)
